...
Runs a focused technical SEO audit for a given URL and returns
...
USER_AGENT = "SEOAuditAgent/1.0 (+https://example.com/seo-audit)"
DEFAULT_TIMEOUT = 12
...
class SeoAuditAgent:
    ...
    def audit(self, url: str, goal: str) -> str:
        ...
        html, final_url, headers, fetch_error = self._fetch(normalized_url)
        if fetch_error:
            return self._render_unreachable(normalized_url, goal, fetch_error)

        analyzer = SimpleHTMLAnalyzer()
        try:
            analyzer.feed(html)
        except Exception as exc:  # pragma: no cover - defensive
            return self._render_unreachable(normalized_url, goal, f"HTML parsing failed: {exc}")

        robots_txt, robots_error, sitemap_urls = self._load_robots_and_sitemaps(final_url)
        ...
        return self._render_report(context, goal, issues)
    ...
    def _fetch(self, url: str) -> Tuple[str, str, Dict[str, str], Optional[str]]:
    ...
    def _load_robots_and_sitemaps(self, url: str) -> Tuple[Optional[str], Optional[str], List[str]]:
        ...
        try:
            with urllib.request.urlopen(req, timeout=DEFAULT_TIMEOUT, context=context) as resp:
                ...
                sitemap_urls = self._extract_sitemaps_from_robots(body)
                return body, None, sitemap_urls
        except Exception as exc:
            return None, str(exc), []
    ...
    def _render_unreachable(self, url: str, goal: str, error: str) -> str:
    ...
    def _render_report(self, context: AuditContext, goal: str, issues: List[Issue]) -> str:
        ...
        for group in grouped.values():
            group.sort(key=lambda i: i.title)
        ...
        lines.extend(self._render_issue_group(grouped["critical"]))
        lines.append("")
        lines.append("2. Important Optimizations – fix soon (medium impact)")
        lines.extend(self._render_issue_group(grouped["important"]))
        lines.append("")
        lines.append("3. Recommended Enhancements – nice to have")
        lines.extend(self._render_issue_group(grouped["recommended"]))

        return "\n".join(lines)

    def _render_issue_group(self, issues: List[Issue]) -> List[str]:
        if not issues:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--insecure", action="store_true", help="Skip SSL certificate verification (use only if cert errors block auditing).")
